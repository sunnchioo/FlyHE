...
void CKKSEvaluator::print_decoded_pt(PhantomPlaintext &pt, int num, std::string str) {
    cudaStreamSynchronize(pt.data_ptr().get_stream());
...
void CKKSEvaluator::print_decrypted_ct(PhantomCiphertext &ct, int num) {
    ...
    cout << "modulus: " << ct.coeff_modulus_size() << " ";
    cout << "data: ";
    ...
    cout << endl;
    // FileIO::SaveText("/mnt/data2/home/syt/data/fhec/log/v" + std::to_string(num) + ".txt", v);
}

double CKKSEvaluator::print_decrypted_ct_min(PhantomCiphertext &ct, int num, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
...
void CKKSEvaluator::print_decrypted_ct(PhantomCiphertext &ct, int num, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
...
void CKKSEvaluator::print_decrypted_ct(PhantomCiphertext &ct, int num0, int num1, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
    ...
    std::cout << "... ";
    for (int i = v.size(); i < num1; i++) {
        cout << v[i] << " ";
    }
    cout << endl;
}

void CKKSEvaluator::print_decrypted_nodecode_ct(PhantomCiphertext &ct, int num, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
    ...
    for (int i = 0; i < num; i++) {
        cout << "(" << v[i].real() << ", " << v[i].imag() << "j) ";
    }
    cout << endl;
}

void CKKSEvaluator::print_decrypted_nodecode_ct(PhantomCiphertext &ct, int num0, int num1, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
    ...
    std::cout << "... ";
    for (int i = v.size() - num1; i < v.size(); i++) {
        cout << "(" << v[i].real() << ", " << v[i].imag() << "j) ";
    }
    cout << endl;
}

void CKKSEvaluator::print_decrypted_ct(PhantomCiphertext &ct, bool iscomplex, int num, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
...
void CKKSEvaluator::print_decrypted_ct(PhantomCiphertext &ct, bool iscomplex, int num0, int num1, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
...
void CKKSEvaluator::print_decrypted_ct_matirx(PhantomCiphertext &ct, int rows, int cols, std::string str) {
    cudaStreamSynchronize(ct.data_ptr().get_stream());
...
void CKKSEvaluator::print_vector(vector<double> &in, int num, std::string str) {
    cout << str << ": ";
...
void CKKSEvaluator::re_encrypt(PhantomCiphertext &ct) {
    ...
    decryptor.decrypt(ct, temp);
    encoder.decode(temp, v);
    encoder.encode(v, ct.chain_index(), ct.scale(), temp);
...
void rlwe::Evaluator::add_inplace_reduced_error(PhantomCiphertext &encrypted1, const PhantomCiphertext &encrypted2) {
...
void rlwe::Evaluator::sub_inplace_reduced_error(PhantomCiphertext &encrypted1, const PhantomCiphertext &encrypted2) {
    size_t encrypted1_coeff_modulus_size = encrypted1.coeff_modulus_size();
    size_t encrypted2_coeff_modulus_size = encrypted2.coeff_modulus_size();
...
void rlwe::Evaluator::multiply_inplace_reduced_error(PhantomCiphertext &encrypted1, const PhantomCiphertext &encrypted2, const PhantomRelinKey &relin_keys) {
    size_t encrypted1_coeff_modulus_size = encrypted1.coeff_modulus_size();
    size_t encrypted2_coeff_modulus_size = encrypted2.coeff_modulus_size();
