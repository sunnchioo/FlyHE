...
void example_ckks_mul(PhantomContext& context, const double& scale) {
    ...
    cout << "    + Scale of x rescale: " << x_cipher.scale() << " bits" << endl;

    cout << context.get_context_data(0).chain_index() << endl;
...
void example_ckks_other(PhantomContext& context, const double& scale) {
    std::cout << "Example: CKKS Other test" << std::endl;
    ...
    for (size_t i = 0; i < x_size; i++) {
        ...
        rand_real = 0.8;
        rand_imag = 0.8;
        x_msg.push_back(make_cuDoubleComplex(rand_real * 1.1, rand_imag * 1.1));
        y_msg.push_back(make_cuDoubleComplex(rand_real * 1.1, rand_imag * 1.1));
    }
    cout << "Message vector: " << endl;
    print_vector(x_msg, 3, 7);
    ...
    cout << "decode after encode." << endl;
    auto result_decode = encoder.decode<cuDoubleComplex>(context, x_plain);
    cout << "Result decode vector: " << endl;
    print_vector(result_decode, 3, 7);
    ...
    PhantomPlaintext x1_plain = secret_key.decrypt(context, x_cipher);
    auto x1_plain_result = encoder.decode<cuDoubleComplex>(context, x1_plain);
    cout << "Result decrypt decode vector: " << endl;
    print_vector(x1_plain_result, 3, 7);

    cout << "Compute x*const." << endl;
    PhantomCiphertext x2_cipher = multiply_const(context, x_cipher, 2);
    PhantomPlaintext x2_plain = secret_key.decrypt(context, x2_cipher);
    auto result = encoder.decode<cuDoubleComplex>(context, x2_plain);
    cout << "Result vector: " << endl;
    print_vector(result, 3, 7);

    PhantomCiphertext xneg2_cipher = multiply_const(context, x_cipher, -2);
    PhantomPlaintext xneg2_plain = secret_key.decrypt(context, xneg2_cipher);
    auto negresult = encoder.decode<cuDoubleComplex>(context, xneg2_plain);
    cout << "Result vector: " << endl;
    print_vector(negresult, 3, 7);

    std::cout << "Example: CKKS add/sub test" << std::endl;
    PhantomCiphertext x_add_cipher = add_const(context, x_cipher, 3);
    PhantomPlaintext x_add_plain = secret_key.decrypt(context, x_add_cipher);
    auto x_add_result = encoder.decode<cuDoubleComplex>(context, x_add_plain);
    cout << "Result vector: " << endl;
    print_vector(x_add_result, 3, 7);

    PhantomCiphertext x_sub_cipher = add_const(context, x_cipher, -0.5);
    PhantomPlaintext x_sub_plain = secret_key.decrypt(context, x_sub_cipher);
    auto x_sub_result = encoder.decode<cuDoubleComplex>(context, x_sub_plain);
    cout << "Result vector: " << endl;
    print_vector(x_sub_result, 3, 7);

    std::cout << "Example: CKKS image unit test" << std::endl;
    PhantomCiphertext x_mult_image_unit_cipher = mult_image_unit(context, x_cipher);
    PhantomPlaintext x_mult_image_unit_plain = secret_key.decrypt(context, x_mult_image_unit_cipher);
    auto x_mult_image_unit_result = encoder.decode<cuDoubleComplex>(context, x_mult_image_unit_plain);
    cout << "Result vector: " << endl;
    print_vector(x_mult_image_unit_result, 3, 7);

    std::cout << "Example: CKKS mod rasing test" << std::endl;
    int level = x_cipher.coeff_modulus_size() - 1;
    if (level > 0) {
        for (int i = level; i > 0; i--) {
            // std::cout << "ckks_switch_modrasing i: " << i << std::endl;
            x_cipher = multiply(context, x_cipher, x_cipher);
            relinearize_inplace(context, x_cipher, relin_keys);
            rescale_to_next_inplace(context, x_cipher);
        }
    }
    // std::cout << "ckks_switch_modrasing 10" << std::endl;

    PhantomCiphertext x_rasing_cipher = format_switch_modrasing(context, x_cipher);
    PhantomPlaintext x_rasing_plain = secret_key.decrypt(context, x_rasing_cipher);
    auto x_rasing_result = encoder.decode<cuDoubleComplex>(context, x_rasing_plain);
    cout << "Result vector: " << endl;
    print_vector(x_rasing_result, 3, 7);
